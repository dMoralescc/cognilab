...
import { empathy } from '@cognilab/shared';
...
export function EmpathyPlayer({ level, seed, onComplete }: Props) {
  const [{ stimuli }] = useState(() => empathy.generate(level, seed));
  const [sIdx, setSIdx] = useState(0);
  const [responses, setResponses] = useState<empathy.EmpathyResponse>([]);
  ...
  useEffect(() => {
    if (!done) return;
    const s = empathy.summarize(stimuli, responses);
  ...
  return (
    <div className="select-none">
      <div className="mb-2 flex items-center justify-between text-xs text-gray-400">
        <span>¿Cuál es la respuesta más empática?</span>
        <span className="font-mono">{sIdx + 1} / {stimuli.scenarios.length}</span>
      </div>
      <div className={`mb-4 rounded-2xl border-2 p-4 ${bg}`}>
        <p className="mb-2 text-sm text-gray-700">{sc.situation}</p>
        <p className="text-xs text-gray-400 italic">La persona siente: {sc.personEmotion}</p>
      </div>
