...
import { personalOrientation } from '@cognilab/shared';
...
export function PersonalOrientationPlayer({ level, seed, onComplete }: Props) {
  const [{ stimuli }] = useState(() => personalOrientation.generate(level, seed));
  const [qIdx, setQIdx] = useState(0);
  const [responses, setResponses] = useState<personalOrientation.PersonalOrientationResponse>([]);
  ...
  return (
    <div className="select-none">
      <div className="mb-3 flex items-center justify-between text-xs text-gray-400">
        <span className="rounded-full bg-purple-50 px-2 py-0.5 text-purple-600 font-medium">Orientación personal</span>
      ...
      <div className="grid grid-cols-1 gap-2">
        {q.options.map((opt, i) => (
          <button key={i} onClick={() => respond(i)} disabled={feedback !== null}
            className="rounded-xl border-2 border-gray-200 bg-white px-4 py-3 text-left text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-60">
            {opt}
          </button>
        ))}
      </div>
