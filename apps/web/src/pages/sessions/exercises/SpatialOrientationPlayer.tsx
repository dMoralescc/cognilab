...
import { spatialOrientation } from '@cognilab/shared';
...
export function SpatialOrientationPlayer({ level, seed, onComplete }: Props) {
  const [{ stimuli }] = useState(() => spatialOrientation.generate(level, seed));
  const [qIdx, setQIdx] = useState(0);
  const [responses, setResponses] = useState<spatialOrientation.SpatialOrientationResponse>([]);
  ...
  useEffect(() => {
    if (!done) return;
    const s = spatialOrientation.summarize(stimuli, responses);
  ...
  return (
    <div className="select-none">
      <div className="mb-3 flex items-center justify-between text-xs text-gray-400">
        <span className="rounded-full bg-indigo-50 px-2 py-0.5 text-indigo-600 font-medium">Orientación espacial</span>
